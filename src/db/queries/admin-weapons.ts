...
export async function createWeapon(editionId: string, data: WeaponFormData): Promise<{ data?: Weapon; error?: string }> {
  try {
    const [created] = await db.insert(weapons).values({ editionId, ...data }).returning();
