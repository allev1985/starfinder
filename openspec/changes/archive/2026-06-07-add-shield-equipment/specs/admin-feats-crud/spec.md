...
- **THEN** all feat records for the edition are displayed with Combat Feat and Shield Prof columns

### Requirement: Admin can create and edit feats
"Add Feat" SHALL open a dialog. Required fields: Name (text, must be unique), Description (textarea). Optional fields: Prerequisites (text), Is Combat Feat (checkbox), Is Shield Proficiency Feat (checkbox), Source Book (text). Edit pre-fills the dialog.
...
#### Scenario: Shield proficiency feat flag stored correctly
- **WHEN** admin checks "Is Shield Proficiency Feat" and submits
- **THEN** the feat row is stored with is_shield_proficiency = true
