...
The Equipment subsection in the Inventory section SHALL group items into three sub-groups rendered in order: "Shields" (category `shield`), "Augmentations & Upgrades" (categories `augmentation_cybernetic`, `augmentation_biotech`, `personal_upgrade`), and "Ammunition" (category `ammunition`). Each sub-group SHALL show a placeholder when empty. The picker SHALL allow filtering by category with tabs: All, Shields, Augments, Ammo.
...
- **WHEN** a character has a shield, a cybernetic augmentation, and a battery in inventory
- **THEN** the shield appears under Shields, the augmentation under Augmentations & Upgrades, and the battery under Ammunition

#### Scenario: Shields picker tab filters to shields only
- **WHEN** the owner opens the equipment picker and selects the Shields tab
- **THEN** only items with category shield are shown
