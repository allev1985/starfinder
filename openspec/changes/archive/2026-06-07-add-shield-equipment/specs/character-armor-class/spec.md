...
- `shieldEacBonus` is `equippedShield.eac_bonus` when the character has `hasShieldProficiency = true` and a shield is wielded, otherwise 0
- `effectiveDex` is `min(dexMod, effectiveMaxDex)` where `effectiveMaxDex` is the lower of `armor.max_dex_bonus` and `equippedShield.max_dex_bonus` when both are non-null; if only one is non-null that value is used; if neither has a cap, dexMod is uncapped
- When no armor is equipped, `armor.eac_bonus` is 0 and armor's max DEX cap is ignored

The total SHALL NOT be stored in the database.

#### Scenario: EAC total includes shield bonus when proficient
- **WHEN** a character has armor with eac_bonus 3, a wielded shield with eac_bonus 1, the Shield Proficiency feat, dex_score 14 (modifier +2), and eac_misc_mod 0
- **THEN** the EAC total displayed is 10 + 3 + 1 + 2 + 0 = 16

#### Scenario: EAC total excludes shield bonus when not proficient
- **WHEN** a character wields a shield with eac_bonus 1 but does not have the Shield Proficiency feat
- **THEN** the shield contributes 0 to EAC total

#### Scenario: EAC total reflects all components with DEX cap
- **WHEN** a character has dex_score 18 (modifier +4), equipped armor with eac_bonus 3 and max_dex_bonus 2, and eac_misc_mod 1, and no shield wielded
- **THEN** the EAC total displayed is 10 + 3 + 0 + 2 + 1 = 16 (DEX capped at +2)

#### Scenario: EAC DEX cap uses lower of armor and shield caps
- **WHEN** armor has max_dex_bonus 4 and the wielded shield has max_dex_bonus 2, dex_score 18 (modifier +4)
- **THEN** effectiveDex is 2 (shield cap is lower)

#### Scenario: EAC total with no DEX cap
- **WHEN** a character has dex_score 18 (modifier +4), equipped armor with eac_bonus 1 and max_dex_bonus null, and no shield, and eac_misc_mod 0
- **THEN** the EAC total displayed is 10 + 1 + 0 + 4 + 0 = 15

#### Scenario: EAC total with no armor equipped
- **WHEN** a character has no armor equipped, no shield wielded, dex_score 14 (modifier +2), and eac_misc_mod 0
- **THEN** the EAC total displayed is 10 + 0 + 0 + 2 + 0 = 12
...
The character sheet SHALL display a KAC total equal to `10 + armor.kac_bonus + shieldKacBonus + effectiveDex + kac_misc_mod`, applying the same shield proficiency check and DEX cap logic as EAC. The total SHALL NOT be stored in the database.

#### Scenario: KAC total includes shield bonus when proficient
- **WHEN** a character has armor with kac_bonus 5, a wielded shield with kac_bonus 2, the Shield Proficiency feat, dex_score 14 (modifier +2), and kac_misc_mod 0
- **THEN** the KAC total displayed is 10 + 5 + 2 + 2 + 0 = 19

#### Scenario: KAC total excludes shield bonus when not proficient
- **WHEN** a character wields a shield with kac_bonus 2 but does not have the Shield Proficiency feat
- **THEN** the shield contributes 0 to KAC total

#### Scenario: KAC total reflects all components with DEX cap
- **WHEN** a character has dex_score 18 (modifier +4), equipped armor with kac_bonus 5 and max_dex_bonus 2, and kac_misc_mod 0, and no shield
- **THEN** the KAC total displayed is 10 + 5 + 0 + 2 + 0 = 17

#### Scenario: KAC total with no armor equipped
- **WHEN** a character has no armor equipped, no shield wielded, dex_score 14 (modifier +2), and kac_misc_mod 0
- **THEN** the KAC total displayed is 10 + 0 + 0 + 2 + 0 = 12
