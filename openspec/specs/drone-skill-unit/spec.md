...
The new character form SHALL require the owner to choose a Skill Unit skill when creating a drone character. The Skill Unit selector SHALL list the drone allowed skills minus any skill already provided by the selected chassis bonus.
...
When a drone character is created, `character_skills` SHALL contain exactly the Skill Unit skill plus the chassis bonus skill (if the chassis has one).
...
The drone skills section SHALL provide an "Add Skill" button opening a dialog filtered to the drone allowed skill list. Skills already present on the drone SHALL be pre-checked. The owner can add skills to represent mod grants, and remove skills to change the Skill Unit.
...
#### Scenario: Owner can remove a skill to change Skill Unit
- **WHEN** the owner clicks the remove control on a drone skill row
- **THEN** the corresponding `character_skills` row is deleted and the owner can add the new Skill Unit via the dialog
