...
The system SHALL define a Postgres enum `race_type` with values `'biological'` and `'android'`. This enum SHALL be used as the column type wherever a race category is stored.
...
The system SHALL provide a query function `getDescriptionsForType(raceType: 'biological' | 'android')` that returns all `race_descriptions` rows for that type ordered by `sort_order` ascending.
