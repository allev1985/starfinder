...
The character owner SHALL be able to select an armor from a searchable combobox on the character sheet. Selecting an armor SHALL persist `equipped_armor_id` on the character immediately (no debounce — it is a discrete selection, not a continuous input). The combobox SHALL only display armor types the character's class is proficient with.
